...
export function mountAWOS(container,onData,onClear){
  container.innerHTML='<div class="panel-heading"><h2>WBGT data source</h2></div><label>Source<select id="weather-source"><option value="manual">On-site sensor readings</option><option value="awos">AWOS / weather station estimate</option></select></label><div id="weather-controls" hidden><div class="weather-fetch"><label>Station<select id="weather-station"><option value="TJSJ">TJSJ - LMMIA, Carolina PR</option><option value="TJBQ">TJBQ - Ramey, Aguadilla PR</option><option value="TJPS">TJPS - Mercedita, Ponce PR</option><option value="TISX">TISX - St Croix ANG</option></select></label><button type="button" id="weather-load">Load latest weather</button></div><p id="weather-status" role="status"></p><p class="weather-assumptions">Estimated outdoor WBGT using the reference’s Liljegren model. Assumes wind measured at 10 m, urban terrain, and solar radiation estimated from time, location and first cloud layer. Nearby airport conditions may differ from the work site.</p><p class="weather-credit">Weather observations: <a href="https://synopticdata.com/" target="_blank" rel="noopener">Synoptic Data</a>. Model adapted from the supplied 121 BEE WBGT page.</p></div>';
